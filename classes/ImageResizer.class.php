...
class ImageResizer extends FileName
{
    ...
    function saveImage($crop=false)
    {
        ...
        switch (strtolower($this->savetype)) { 
            ...
            case('jpg'): case('jpeg'):
                imagejpeg($this->newimage,$this->savedirectory.'/'.((!is_null($this->newImageNewName)) ? $this->newImageNewName : $this->filename).$this->suffix.'.'.$this->savetype,$this->quality);
                break;

            case('gif'):
                imagegif($this->newimage,$this->savedirectory.'/'.((!is_null($this->newImageNewName)) ? $this->newImageNewName : $this->filename).$this->suffix.'.'.$this->savetype,$this->quality);
                break;

            case('png'):
                imagepng($this->newimage,$this->savedirectory.'/'.((!is_null($this->newImageNewName)) ? $this->newImageNewName : $this->filename).$this->suffix.'.'.$this->savetype,$this->quality);
                break;
    ...
    function resizeToFill($crop=false){
        ...
        return count($this->errorArr) ? $this->displayErrors() : $this->saveImage($crop);
    }
    ...
    function resizeToFit($dontenlarge=false){
        ...
        return count($this->errorArr) ? $this->displayErrors() : $this->saveImage();
    }
    ...
    function displayErrors(){
        if(count($this->errorArr)){
            $str = 'The image could not be resized for the following reason'.(count($this->errorArr)>1?'s':'').'... <br/><ul>';
