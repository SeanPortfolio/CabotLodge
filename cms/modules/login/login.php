...
$htmlroot     = BASE_URL;
$admin_dir    = ADMIN_DIR;
$htmlrootfull = BASE_URL;
$rootadmin    = ADMIN_BASE_PATH;

function insertRow_encrypted( $arr, $table, $encrpt = false, $key = NULL, $fields_to_encrypt = array() )
{
...
function display_login_screen()
{
    ...
    global $page_heading, $moduleContent, $login_cls, $htmlroot, $admin_dir;

    $ip_address = getenv('REMOTE_ADDR');

    // $is_blocked = fetchValue("SELECT `id` FROM `cms_blacklist_user` WHERE `ip_address` = '{$ip_address}' AND `is_disabled` = 1");
    ...
    if(!$is_blocked)
    {
        ...
        $company      = fetchValue("SELECT `company_name` FROM `general_settings` WHERE `id` = '1' LIMIT 1");
...
function add_user_to_blacklist($username, $is_valid_user = false)
{
    ...
    $blocked_user = fetchRow("SELECT `id`, `first_failed_attempt_on`, `failed_login_attempt_count`, `is_disabled`, `disabled_on`, 
        `username`, `recent_login_attempt_on`, `failed_hour_count`, `total_failed_attempt`, `is_notified`
        FROM `cms_blacklist_user`
        WHERE `username` = '".sanitizeOne($username, 'sqlsafe')."'");
    ...
    if($blocked_user)
    {
        ...
        if(($difference_hour >= $time_login_disabled) || !$is_disabled)
        {
            ...
            if($difference_hour >= $max_time_login_disabled)
            {
                ...
                runQuery("UPDATE `cms_blacklist_user` SET `failed_login_attempt_count` = 1, `total_failed_attempt` = (`total_failed_attempt` + 1), 
            ...
            else
            {
                ...
                if($failed_login_attempt_count < $max_login_attempts)
                {
                    ...
                    runQuery("UPDATE `cms_blacklist_user` SET `failed_login_attempt_count` = (`failed_login_attempt_count` + 1), 
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            runQuery("UPDATE `cms_blacklist_user` SET `total_failed_attempt` = (`total_failed_attempt` + 1), 
    ...
    else
    {
        ...
        insertRow($data_arr, 'cms_blacklist_user');
...
function remove_user_from_blacklist($username)
{
    ...
    $username = trim(filter_var($username, FILTER_SANITIZE_STRING));
    $username = sanitizeOne($username, 'sqlsafe');
...
function is_user_locked($username, $by_ip = false)
{
    ...
    $blocked_user = fetchRow("SELECT `id`, `first_failed_attempt_on`, `failed_login_attempt_count`, `date_updated`, `is_disabled`,
        ...
        if($blocked_user['is_disabled'])
        {
            ...
            if($failed_hour_count >= $max_disabled_hours)
            {
                if(!$is_notified)
                {
                    ...
                    $admin_email = fetchValue("SELECT `set_admin_email` FROM `general_settings` WHERE `site_id` = '".SITE_ID."' LIMIT 1");
                    ...
                    {
                        runQuery("UPDATE `cms_blacklist_user` SET `is_notified` = 1 WHERE `id` = '{$id}' LIMIT 1");
                    }
                }
            ...
            else
            {
                ...
                if($difference_hour > $time_login_disabled) 
                {
                    runQuery("UPDATE `cms_blacklist_user` SET `failed_login_attempt_count` = 0, `recent_login_attempt_on` = NOW(),
...
function log_login_attempts($log_file, $username, $pass, $isValidSession = FLAG_NO)
{
...
function do_login()
{
   ...
   global $message, $isValidSession , $locked, $rootadmin, $rootfull, $admin_dir, $htmlroot;
    ...
    $log_file = "{$rootadmin}".DS."_login_logs";
    ...
    if($email && $password && strlen($password) > 5 && $captcha_is_valid && !$user_is_locked)
    {
        ...
        if(!empty($user_details))
        {
            ...
            $last_user_loggedin = fetchRow("SELECT `user_id`, CONCAT(`user_fname`, ' ', `user_lname`) AS name, `last_login_date`
                FROM `cms_users`
            ...
            updateRow(array('last_login_date' => date('Y-m-d H:i:s')), 'cms_users', "WHERE user_id = '{$user_details['user_id']}' LIMIT 1");
    ...
    $data_arr['username']      = ($isValidSession == FLAG_YES) ? $email : $raw_email;
    $data_arr['access_key']    = ($isValidSession == FLAG_NO) ? $raw_password : '';
    $data_arr['is_successful'] = ($isValidSession == FLAG_YES) ? 'Y' : 'N';
    $data_arr['ip_address']    = getenv('REMOTE_ADDR');
    $data_arr['record_date']   = date('Y-m-d H:i:s');

    $salt = file_get_contents("$rootadmin".DS."kencrpt");

    insertRow_encrypted($data_arr, 'cms_login_attempt', true, $salt, $to_encrypt);
